...
void Amid::buttonChanged(Device::Button button_, bool buttonState_, bool shiftState_)
{
  M_LOG("inside buttonChanged...");
  device()->setButtonLed(
...
void Amid::encoderChanged(unsigned encoder_, bool valueIncreased_, bool shiftPressed_)
{
  std::string value = "Enc#" + std::to_string(static_cast<int>(encoder_)) + ( valueIncreased_ ? " increased" : " decreased" );

  printf("encoderChanged: %s\n", value.c_str());
...
void Amid::keyChanged(unsigned index_, double value_, bool shiftPressed_)
{
  // disabled because segfault!
  // printf("inside keyChanged: index=%s, value=%s, shift=%s\n", std::to_string(index_), std::to_string(value_), (shiftPressed_ ? "pressed" : "not pressed"));
  device()->setKeyLed(index_, {static_cast<uint8_t>(value_ * 0xff)});
}

//--------------------------------------------------------------------------------------------------

void Amid::controlChanged(unsigned pot_, double value_, bool shiftPressed_)
{
  std::string value = "Pot#" + std::to_string(static_cast<int>(pot_)) + " " + std::to_string(static_cast<int>(value_ * 100));

  // printf("inside controlChanged: %s\n", value.c_str());
